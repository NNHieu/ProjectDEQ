...
class Direction:
    ...
    def _normalize(direction: Tensor, weights: Tensor, norm="filter"):
        ...
        if norm == "filter":
            ...
            sc = weights.norm(dim=(-1, -2), keepdim=True) / (
                direction.norm(dim=(-1, -2), keepdim=True) + 1e-10
            )
            direction.mul_(sc)
        elif norm == "layer":
...
class Dir2D(object):
    ...
    def similarity(self):
        return proj.cal_angle(
            proj.nplist_to_tensor(self._dir[0]), proj.nplist_to_tensor(self._dir[1])
        )
...
class Sampler:
    def __init__(
        self, model, surface, layer_names, device, comm=None, rank=-1, logger=None
    ) -> None:
        self.model = model
    ...
    def run(self, evaluate, inds, coords, max_inds_nums):
        ...
        with torch.no_grad():
            ...
            for count, ind in enumerate(inds):
                ...
                log_values = "\t".join(
                    ["{}={:.3f}".format(name, val) for name, val in zip(self.layer_names, values)]
                )
                self.logger.info(
        ...
        self.logger.info(
            "Rank %d done!  Total time: %.2f Sync: %.2f" % (self.rank, total_time, total_sync)
        )
